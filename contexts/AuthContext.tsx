...
interface AuthContextType {
  ...
  azurirajKorisnickePodatke: (
    podaci: Partial<KorisnickiPodaci>,
  ) => Promise<void>;
}
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [korisnickiPodaci, setKorisnickiPodaci] =
    useState<KorisnickiPodaci | null>(null);
    ...
    try {
      const unsubscribe = onAuthStateChanged(auth, async (firebaseKorisnik) => {
        try {
          ...
          if (firebaseKorisnik) {
            try {
              const korisnickiDokument = await getDoc(
                doc(db, "users", firebaseKorisnik.uid),
              );
              if (korisnickiDokument.exists()) {
                ...
                setKorisnickiPodaci({
                  ...
                  uloga:
                    podaci.uloga ||
                    (podaci.role === "admin" ? "admin" : "korisnik"),
                  ime: podaci.ime || podaci.name,
                  telefon: podaci.telefon || podaci.phone,
                  adresa: podaci.adresa || podaci.address,
                  omiljeniProizvodi:
                    podaci.omiljeniProizvodi || podaci.favoriteProducts || [],
                  istorijaPorudzbina:
                    podaci.istorijaPorudzbina || podaci.orderHistory || [],
                } as KorisnickiPodaci);
              } else {
  ...
  const registracija = async (email: string, lozinka: string, ime: string) => {
    ...
    const korisnickiKredencijali = await createUserWithEmailAndPassword(
      auth,
      email,
      lozinka,
    );
  ...
  const azurirajKorisnickePodatke = async (
    podaci: Partial<KorisnickiPodaci>,
  ) => {
    if (!korisnik || !auth) return;

    const azuriraniPodaci = { ...korisnickiPodaci, ...podaci };
    await setDoc(doc(db, "users", korisnik.uid), azuriraniPodaci, {
      merge: true,
    });
